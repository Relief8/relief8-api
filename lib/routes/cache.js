...
module.exports = function* () {
    ...
    var survivors = yield db.survivors.find({ location: { $geoWithin: { $centerSphere: [[lng, lat], radius] } }, date: { $gt: cutoffPeriod } }, { sort: { _id: -1 } });

    // Got any?
    if (survivors.length === 0) {
        throw { status: 400, message: 'No survivors were found in a radius of ' + config.cache.radiusMiles + ' miles from you.\n\nPlease ask the creator of the group for the group link instead.' };
    }
